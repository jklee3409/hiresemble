...
- LLM 심사: 채용 담당자 역할의 심사 모델이 두 답변을 A/B로만 보고(어느 쪽이 Hiresemble인지 숨기고 케이스마다 순서를 바꿈) 질문 적합성·구체성·개인 기여·직무/회사 적합성·신뢰성·가독성을 1~5점으로 매기고 면접에 올릴 답변을 고른다.
...
$env:COVER_LETTER_EVAL_JUDGE_MODEL = "gpt-5.6-sol"    # 선택, 기본은 평가 모델과 같음
...
- `backend/build/reports/cover-letter-eval/report.md`: 승/패/무, 평균 점수, 평균 채움 비율, 근거 없는 수치 합계, 케이스별 표
- `backend/build/reports/cover-letter-eval/report.json`: 두 답변 원문, 심사 근거, 기준별 점수와 결정적 지표. 사람이 다시 채점할 때 이 파일을 쓴다.

평가 데이터는 모두 가상이지만, 리포트를 공유할 때 실제 지원자 자료를 케이스에 넣지 않았는지 확인한다.

## 해석 주의

- LLM 심사는 같은 계열 모델의 문체를 선호하는 편향이 있을 수 있다. 심사 모델을 바꿔 두 번 실행하거나 `report.json`으로 사람이 교차 채점한다.
