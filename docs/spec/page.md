...
```text
[상단] 작업 Header(공고 · 제목 · 상태 · 답변/검증 진행)
[상단] AI 코치 Panel(현재 상태 문장 · 단일 primary 다음 행동 · 5단계 진행)
[좌측] 문항 Navigator
[중앙] 문항 등록 Form / 선택 문항 Brief / TipTap 답변 Editor
[우측] 1 공고 요구사항 → 2 강점·보완점 → 3 소재 선택 → 4 AI 초안 요청 → 5 검증 결과
[하단] 버전 이력 · 최종화 확인
```

AI 코치 Panel은 새 API를 만들지 않고 자기소개서 상세, 최신 공고 분석, 확인한 경험, Agent Run 상태에서 유도한 문장과 단계만 보여 준다. 화면의 primary action은 코치 Panel 하나가 소유하고 나머지 실행 button은 보조 위계를 유지한다. 오른쪽 Panel의 1~5 번호는 코치 Panel의 단계 번호와 같은 순서를 가리킨다.
...
문항 등록 form은 문항 Navigator의 `문항 추가`로 열고 중앙 열에 표시한다. 문항 내용·최대 글자 수·메모 수정은 선택 문항 Brief 안의 접힌 disclosure에서 제공한다.
