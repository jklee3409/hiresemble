...
[4] 우: 작성 도움 tab(쓸 소재 / 공고 요구사항 / AI 검토 결과)
...
AI 설정은 기본 화면에 펼쳐 두지 않고 `AI 초안 만들기`·`AI로 다시 쓰기`를 눌렀을 때 sheet로 연다. 이미 답변이 있으면 문구를 `AI로 다시 쓰기`로 바꾸고 지금 답변이 지워지지 않고 새 버전으로 저장된다는 점, 이전 버전을 되돌릴 수 있다는 점, 저장하지 않은 내용이 있다는 점을 실행 전에 알린다. 작성 도움은 `쓸 소재`, `공고 요구사항`, `AI 검토 결과` tab으로 나누고 한 번에 하나만 보여 준다. 소재 tab은 고른 소재가 다음 AI 초안에 쓰인다는 점, 선택 개수와 모두 해제, 아직 쓰지 않은 소재와 이 답변에 이미 쓴 소재를 구분해 보여 주며 확인한 경험이 없으면 자료 등록 경로를 제공한다. 공고 요구사항 tab은 요구 역량과 보완점을 담고 소재 선택과 섞지 않는다. 숫자만 나열하지 않는다. 버전 기록과 작성 완료 점검도 sheet로 열며, 작성 완료 조건은 상단 요약에서 언제든 확인하고 부족한 조건마다 해당 문항으로 이동할 수 있다. 이 화면들은 새 API를 만들지 않고 자기소개서 상세, 최신 공고 분석, 확인한 경험, Agent Run 상태에서 유도한 정보만 사용한다.

AI 작업 진행은 기본적으로 한 줄 요약(진행 문구·상태·진행률)만 보여 주는 접힌 disclosure로 두고, 연결 상태·문항별 결과·`AI 작업 상세` link는 펼쳤을 때만 노출한다. 재시도가 필요한 문항이 있으면 처음부터 펼친다. 완료 문항 목록은 질문 전체를 그대로 늘어놓지 않고 문항 번호와 앞부분을 한 줄 preview로 보여 주며 전체 질문은 접근 가능한 이름과 hover title로 보존한다.
