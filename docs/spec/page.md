...
AI 설정은 기본 화면에 펼쳐 두지 않고 `AI 초안 만들기`·`AI로 다시 쓰기`를 눌렀을 때 sheet로 연다. 이미 답변이 있으면 문구를 `AI로 다시 쓰기`로 바꾸고 지금 답변이 지워지지 않고 새 버전으로 저장된다는 점, 이전 버전을 되돌릴 수 있다는 점, 저장하지 않은 내용이 있다는 점을 실행 전에 알린다. 답변에 쓸 소재는 상단 주요 행동 button 바로 아래의 `답변에 사용할 소재` button에서 펼친다. 이 영역은 화면 가운데 modal이 아니라 button에 붙어 펼쳐지고, 다른 영역 위에 겹쳐 떠서 아래 내용을 밀어내지 않는다. 고른 소재가 다음 AI 초안에 쓰인다는 점, 선택 개수와 모두 해제, 아직 쓰지 않은 소재와 이 답변에 이미 쓴 소재를 구분해 보여 주며 확인한 경험이 없으면 자료 등록 경로를 제공한다. 우측 작성 도움은 `공고 요구사항`과 `AI 검토 결과` tab만 두고 소재 선택과 섞지 않으며, 편집 영역과 같은 높이 안에서만 스크롤한다. 두 tab은 같은 열 폭을 쓰고 tab을 바꿔도 편집기 폭이 달라지지 않는다. 내용도 같은 방식으로 그린다. 제목이 붙은 block 안에 옅은 채움면 카드를 쌓고 카드 위에는 분류나 심각도 알약을 둔다. 검토 결과를 별도의 떠 있는 카드나 modal로 분리하지 않는다. 숫자만 나열하지 않는다. 버전 기록과 작성 완료 점검도 sheet로 열며, 작성 완료 조건은 상단 요약에서 언제든 확인하고 부족한 조건마다 해당 문항으로 이동할 수 있다. 이 화면들은 새 API를 만들지 않고 자기소개서 상세, 최신 공고 분석, 확인한 경험, Agent Run 상태에서 유도한 정보만 사용한다.

AI 작업 진행은 진행 중일 때만 한 줄 요약(진행 문구·상태·진행률) disclosure로 보여 준다. 성공이든 실패든 끝난 작업의 상태 bar는 화면에 남기지 않고, 결과는 답변·검토 상태와 알림으로 전달한다. 결과 알림은 이 화면에서 진행을 지켜본 작업에만 띄워 다시 들어왔을 때 지난 결과가 반복되지 않게 한다. 연결 상태·문항별 결과·`AI 작업 상세` link는 펼쳤을 때만 노출하고 재시도가 필요하면 처음부터 펼친다. 완료 문항 목록은 질문 전체를 그대로 늘어놓지 않고 문항 번호와 앞부분을 한 줄 preview로 보여 주며 전체 질문은 접근 가능한 이름과 hover title로 보존한다.
