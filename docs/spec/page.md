...
참고 자료 세 종류는 문항과 AI 초안 실행 영역보다 먼저 배치한다. 각 항목은 요약 preview와 개수를 가진 3열 dropdown으로 제공하고 기본 상태에서는 접어 두며, 사용자가 펼친 내용은 높이를 제한하고 내부 scroll을 허용해 화면을 과도하게 차지하지 않는다. AI 설정은 disclosure나 select로 감추지 않고 초안을 받을 문항 checkbox, 작성 방식 radio, 문항마다 다른 경험 checkbox를 compact 영역에 바로 보여 준다. AI 코치 Panel은 새 API를 만들지 않고 자기소개서 상세, 최신 공고 분석, 확인한 경험, Agent Run 상태에서 유도한 문장과 단계만 보여 준다. 참고 자료의 1~3 번호는 코치 Panel 단계 번호와 같은 순서를 가리킨다.

AI 초안 생성 결과의 완료 문항 목록은 질문 전체를 그대로 늘어놓지 않고 문항 번호와 앞부분을 한 줄 preview로 보여 준다. 전체 질문은 접근 가능한 이름과 hover title로 보존한다.
...
문항 등록 form은 tab bar의 `문항 추가`로 열고 답변 작업대 위에 표시한다. 문항 내용·글자 수 제한·메모 수정과 문항 삭제는 선택 문항 Brief 안의 접힌 disclosure에서 제공하고, 문항 순서 변경은 선택한 문항의 `앞으로 이동`·`뒤로 이동`으로 수행한다.
