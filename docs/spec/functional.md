...
자격증·어학·수상·경력을 저장하거나 수정하면 서버가 해당 레코드와 연결된 `profile_evidence`를 동기화한다. 학력은 구조화 프로필에서만 관리하며 대외활동 근거를 생성하지 않는다.
...
문서 AI 추출 근거는 `PENDING`으로 시작한다. 사용자가 `VERIFIED`로 승인한 항목만 공고 분석·자기소개서·면접 준비의 근거로 사용하며, 잘못 추출된 항목은 `REJECTED`로 제외한다. 사용자가 구조화 프로필에 직접 입력한 근거는 이미 사용자 확인을 거쳤으므로 기본 `VERIFIED`이고 별도 승인·거절 대상이 아니다. `SOURCE_DELETED` tombstone은 읽기 전용이며 수정·승인·거절할 수 없다.
