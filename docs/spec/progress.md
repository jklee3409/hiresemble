...
## [2026-09-26] Session Summary (공고 분석 분할 매칭 계약)

- What was done:
  - 기능 명세에 문장 단위 원자화·비요건 label 제외와 `MATCH_EVIDENCE` 최대 3묶음 분할·전역 `criterionIndex`·병합 규칙을 추가했다.
- Key decisions:
  - None
- Issues encountered:
  - None
...
- What was done:
  - 기능 명세 source block 규칙에 비채점 heading의 `OTHER` 전환, `label: 내용` 한 줄 처리, `등` 업무 목록 유지를 추가했다.
- Key decisions:
  - None
- Issues encountered:
  - None
- Validation:
  - `JobPostingSectionPolicy` 구현과 문구를 대조했다.
- Next steps:
  - None

## [2026-09-25] Session Summary (공고 마감 시각 KST 해석 계약)

- What was done:
  - `functional.md` 공고 등록 처리에 자동 추출 마감일의 `Asia/Seoul` 기본 해석·날짜만·`24:00`·명시 offset 규칙을, `tech_stack.md` 공고 추출 절에 fields prompt v4·output schema v4와 서버 변환 책임을 추가했다.
- Key decisions:
  - workflow version, DB·API 계약은 유지하고 Provider 출력 schema와 prompt identity만 올린다.
- Issues encountered:
  - None
- Validation:
...
- What was done:
  - 기술 명세에 v5 단계·tier별 reasoning/timeout, 출력 상한, provider timeout 상한 180초, opt-in 품질 평가 태스크를 반영했다.
- Key decisions:
  - None
- Issues encountered:
  - None
...
## [2026-09-24] Session Summary (자기소개서 생성 v5 계약)

- What was done:
  - CL-003 워크플로와 생성 규칙, 기술 명세 workflow 표·모델 선택 절에 v5 단계, 작성 context(공고 분석·회사 조사), 권장 framework, memo 수정 지시, 검토 점수 기록을 반영했다.
- Key decisions:
  - 신규 API·DB 변경 없이 기존 memo와 조회 경계로 구현한다.
- Issues encountered:
  - None
- Validation:
  - Markdown 변경만 수행했다. 구현 검증은 backend 기록을 따른다.
- Next steps:
  - None
...
- Key decisions: 품질 모드 공개 계약은 폐기하고 provider model ID를 서버 allowlist로 검증한다.
- Issues encountered: None.
- Validation: OpenAPI·Backend·Frontend 전체 표준 검증 통과.
- Next steps: 모델 catalog 변경은 API·가격 migration·운영 문서를 함께 갱신한다.
