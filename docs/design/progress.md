...
다섯 P0 승인 명세를 연결한 전체 시스템 설계와 단계별 구현 계획, 승인 결정 기록이 작성되어 있다. P0–P8은 완료됐고 P8.5의 Chat strict output부터 문서 finalize까지 live 증거가 있으나 terminal 보정은 live 재검증 전이다. P8.5-V–P8.9-A가 P9의 선행이며 P10은 사용자 설정, 운영 안정성, 출시 준비로 분리돼 있다. 별도 GitHub·Career Artifact 설계는 Gate 0–4 `DONE`, Gate 5 `IMPLEMENTED_NOT_VERIFIED`, 실제 UAT `USER_MANUAL_UI_VALIDATION_PENDING`을 구분한다. 이 디렉터리는 코드 진행 문서를 대신하지 않는다.

## [2026-09-24] Session Summary (자기소개서 v4 writer 원문 발췌 설계 반영)

- What was done:
  - `system-architecture.md` 11.2에 v4 writer·FactCheck가 배분된 `VERIFIED` 근거의 원본 masked chunk를 서술 맥락으로 받는다는 경계를 추가했다.
- Key decisions:
  - 정량 사실과 claim provenance는 `VERIFIED` 근거 content로만 판단한다.
- Issues encountered:
  - None
- Validation:
  - Markdown 변경만 수행했다.
...
## [2026-08-09] Session Summary (공개 GitHub archive 수집 설계 반영)

- What was done: 공개 repository content snapshot을 고정 codeload origin의 commit-addressed bounded archive로 명시했다.
- Key decisions: archive redirect·SHA·압축/해제·entry 제한과 metadata REST quota를 별도 경계로 기록했다.
- Issues encountered: 없음.
- Validation: 실제 공개 repository archive 수집과 security regression 결과를 설계에 반영했다.
- Next steps: anonymous metadata traffic 용량 계획을 수행한다.

## [2026-08-09] Session Summary (Gate 5 구현 상태와 수명주기 설계 동기화)

- What was done:
  - GitHub App sequence/state/token lifecycle, disconnect/revocation, terminal account purge, feature matrix와 원래 시나리오 1–5의 UI/API/DB/test 추적성을 V29·V30 실제 구현에 맞췄다.
- Key decisions:
  - Backend/Frontend/DB 구현과 실제 외부 UAT를 분리하고, 최종 mocked Chromium 미통과 때문에 Gate 5를 `DONE` 대신 `IMPLEMENTED_NOT_VERIFIED`로 기록했다.
- Issues encountered:
  - 설계의 기존 V28·88/118·79/107 기준선과 planned 상태가 실제 V30·97/127·90/120·81/109와 달라 모두 갱신했다.
- Validation:
  - 코드의 OpenAPI test 상수, migration 파일과 전체 check 결과를 대조했다.
- Next steps:
  - Phase 5 Chromium 재검증 성공 뒤 Gate 5만 `DONE`으로 올리고 외부 UAT는 사용자 결과가 있을 때 별도로 갱신한다.
...
- What was done:
  - GitHub·Career Artifact 설계, 시스템 architecture와 구현 계획을 Gate 0–4 `DONE`, Gate 5 `PLANNED` 실제 상태로 동기화했다.
- Key decisions:
  - Frontend는 Gate 3의 11개 operation과 V28 lifecycle을 그대로 소비하며 private GitHub와 account deletion terminal purge는 후속 경계로 유지한다.
- Issues encountered:
  - 없음.
- Validation:
  - Frontend 전체 check·통합 Chromium, Backend 95/647, Compose와 diff/checksum 결과를 설계 상태 근거로 연결했다.
- Next steps:
  - Gate 5는 별도 승인 전 구현 상태로 승격하지 않는다.

## [2026-08-08] Session Summary (Career Artifact 설계 Gate 3 완료 반영)

- What was done:
  - 상세 설계·시스템 architecture·구현 계획을 V28 Career Artifact Backend, exact model, 두 workflow와 POI/Object lifecycle의 실제 구현에 맞췄다.
- Key decisions:
  - upload는 DB 밖, immutable apply는 checkpoint completion transaction 안에서 수행하고 rollback/cancel cleanup은 deterministic key와 outbox로 보상한다.
  - renderer-only profile과 incidental Context 개인정보를 서로 다른 경계에서 차단하며 Gate 4 UI와 Gate 5 Private GitHub는 계획 상태로 유지한다.
- Issues encountered:
  - POI 표준 document thumbnail과 금지된 slide media를 구분하고, soft delete privacy scrub을 immutable 계약의 유일한 단방향 예외로 문서화했다.
- Validation:
  - V28 migration·workflow·renderer·OpenAPI contract test와 전체 Backend/Frontend 검증 결과를 설계의 Gate matrix와 대조했다.
- Next steps:
  - Gate 4는 11개 기존 operation을 소비하며 공개 API를 임의로 늘리지 않는다.

## [2026-08-08] Session Summary (GitHub 설계 Gate 2 실제 상태 반영)

- What was done:
  - GitHub·Career Artifact 설계, 구현 계획과 시스템 architecture를 feature-gated Gate 2 Frontend 구현 상태에 맞췄다.
- Key decisions:
  - Gate 0–2만 완료로 두고 artifact API·migration·model 선택·DOCX/PPTX·download와 Private GitHub Gate 3–5는 계획 상태로 유지했다.
- Issues encountered:
  - focused Playwright 최종 locator 재확인 필요를 구현 완료와 별도 검증 위험으로 기록했다.
- Validation:
  - 실제 Frontend route/API/feature/Agent Run/provenance 코드와 Backend 79 paths/107 operations·V27 경계를 대조했다.
- Next steps:
...
- What was done:
  - 시스템 구조와 구현 계획에 candidate embedding, exact/semantic match, canonical 저장, 출처 연결과 사용자 해결 흐름을 추가했다.
- Key decisions:
  - 문서 원본 evidence와 사용자 경험 library를 분리해 downstream 중복 사용과 source lifecycle 결합을 해소한다.
- Issues encountered:
  - Frontend 구현은 다른 작업과 충돌하지 않도록 이번 범위에서 제외했다.
- Validation:
  - 구현된 Backend workflow·DB·API와 설계 문서의 version·상태를 대조했다.
- Next steps:
  - 경험 관리 화면과 운영 평가 지표를 후속 단계로 진행한다.
