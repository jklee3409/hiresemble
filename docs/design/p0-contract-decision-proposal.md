...
이 문서는 구현자가 추측하지 않도록 P0 계약의 권장 기준선을 제안한다. 제품 소유자 승인 전에는 아래 내용을 확정 계약으로 보지 않으며, `docs/spec/**`, 비즈니스 코드, migration, 설정, API와 UI에 반영하지 않는다.
...
| D-02 | `RECOMMENDED`             | 공개 품질, 모델 등급, 검색 품질이 혼용되고 version source·질문 유형의 집합과 생성 경로가 다르다.                                | 공개 `AiQualityMode`, 내부 `ModelTier`, `ResearchQuality`, `SourceCoverage`를 분리한다. `CoverLetterVersionSource`와 `InterviewQuestionType`은 3장의 canonical 값으로 통일하고 source 값은 서버 command가 지정한다.                                                                                                                                                          | 공개 계약과 운영 모델 교체를 분리하고 provenance 위조를 막는다.                                | 단일 `QualityMode`, provider/model명을 공개 입력으로 사용, client가 version source 지정             | functional CL/INT/SYS, api cover/interview/settings, db 2/7/8/9, page 8/9/12/13, tech 8/9 | OpenAPI, DB CHECK, Java enum, TS/Zod, ModelRouter                           | 네 계층 enum parity, 허용하지 않은 품질·source 입력 거부, HIGH_QUALITY gate 테스트                   |
...
| D-13 | `OWNER_DECISION_REQUIRED` | signup session/response, 익명 CSRF, profile completion, 탈퇴 삭제 추적과 draft 보존이 불완전하다.                               | 익명 `GET /auth/csrf`, signup/login CSRF와 session rotation, 공통 auth response를 사용한다. profile completion은 서버가 계산한다. 탈퇴 즉시 `WITHDRAWN`+모든 session 폐기+Agent Run과 분리된 durable deletion task의 202 receipt를 권장한다. draft는 사용자/resource별 `sessionStorage`와 폐기 규칙을 적용하되 삭제 SLA·draft 보존 UX는 승인받는다.                          | 인증 흐름을 단일화하고 공용 browser 계정 간 원문 노출과 삭제 유실을 막는다.                    | signup/login CSRF 면제, 204 탈퇴, 사용자 ID 없는 무기한 localStorage                                | functional AUTH/PROF, api auth/account, db user lifecycle, page 3/13/14/15                | Security/session, deletion task, route guard, draft namespace/cache purge   | 익명 CSRF→signup, rotation, 전 session 폐기, receipt 추적, 두 사용자 draft/cache E2E                 |
...
| D-16 | `RECOMMENDED`             | logical step·Agent class 목록, 미승인 chunk, vector index와 URL/search gateway 경계가 다르다.                                   | logical step은 class 이름과 분리하고 6장의 fixed workflow를 따른다. 미승인 masked chunk는 후보 탐색·모순 확인에만 쓰고 writer·score의 긍정 사실 근거로 쓰지 않는다. embedding은 MVP 1536차원으로 고정하고 공고 fetch와 search gateway를 분리한다.                                                                                                                            | 자유 loop, 미승인 사실 생성, mixed vector와 SSRF·비용 경계 혼동을 막는다.                      | Agent class=workflow contract, writer에 raw chunk, Tavily로 사용자 URL fetch, mixed dimension index | functional CL/INT, db chunks/provenance, tech 4/6/8/9, design                             | workflow registry, ContextBuilder, vector/gateway migration                 | context 격리·provenance, prompt injection, dimension boot check, model filter, gateway SSRF          |
| D-17 | `RECOMMENDED`             | `/`, job 기본 child, catch-all, mock 생성 위치와 목록 filter가 없다.                                                            | `/`은 인증·profile 상태에 따라 login/onboarding/dashboard로 보낸다. job 기본 child는 overview, catch-all은 전용 404다. 안전한 `returnTo`와 canonical query를 사용하고 목록 filter는 7장 기준으로 추가한다.                                                                                                                                                                   | open redirect·guard loop·deep-link 단절과 화면/API 불일치를 막는다.                            | referrer/localStorage return, tab local state만 사용, dashboard fan-out                             | page 1/4/7/10/11/15, api list/dashboard, design                                           | router/meta/query schema, list DTO/filter                                   | root 3상태, 악성 return URL, direct child, 404, filter URL E2E                                       |
...
| B    | embedding model·dimension              | 1536 fixed, exact-first, controlled reindex        | boot/dimension/filter/load test           |
...
| `VerificationStatus`         | `PENDING`, `PASSED`, `WARNING`, `FAILED`                                                                                                                   | 읽기                                    | cover verification                     | 한 verification은 PENDING→PASSED/WARNING/FAILED                                                                                                                                                                                                | PASSED/WARNING/FAILED                                            | 재검증은 새 verification/run, cancel은 pending run                                    |
...
| `ModelTier`                  | `LOW_COST`, `BALANCED`, `HIGH_QUALITY` 내부 routing 결과                                                                                                   | run/step 읽기 전용, 입력 금지           | policy·step·usage                      | 호출마다 policy가 선택                                                                                                                                                                                                                         | 해당 없음                                                        | LOW_COST→BALANCED 승격 1회 가능, attempt에 포함                                       |
...
| `InterviewFeedbackStatus` | PENDING, SUCCEEDED, FAILED                                                                                                                    | answer feedback projection/DB; 재시도는 새 immutable feedback            |
...
| `ProfileCompletionItem`   | LEGAL_NAME, DESIRED_ROLE, DESIRED_INDUSTRY, DESIRED_LOCATION, PRIMARY_EDUCATION                                                               | profile/dashboard 공개 계산 projection                                   |
...
- 이미 존재하는 동시 편집 aggregate를 바꾸는 command의 `version`은 0 이상 필수다. delete는 `?version=n`, append-only answer 저장은 `parentVersionId`가 current CAS다. signup/login/logout, display-name/password, 탈퇴처럼 session·credential 또는 단일 사용자 command이고 endpoint matrix가 `version 없음`으로 선언한 요청은 예외이며 자체 transaction·idempotency·현재 credential 검증으로 보호한다.
...
`I` 적용 endpoint는 document upload/manual/reparse, job create/extraction retry/analysis, cover letter create/generate/verify, interview preparation/research retry/answer feedback, mock session create/complete, Agent Run retry, account delete다. 같은 key+hash 완료 응답은 원래 HTTP status와 DTO를 재생하고 `Idempotency-Replayed: true`를 붙인다. validation·owner 실패는 record 생성 전에 반환한다.
...
| `EvidenceRefDto`        | `id:UUID`, `title:string 1..250`, `evidenceCategory:string 1..80`, `verificationStatus:EvidenceVerificationStatus`, `sourceType:string 1..50`, `sourceDeleted:boolean`                                                  |
| `ResearchSourceRefDto`  | `id:UUID`, `topic:ResearchTopic`, `title:string? <=500`, `sourceUrl:string 1..2048`, `sourceType:ResearchSourceType`, `retrievedAt:Instant`                                                                             |
...
| `CurrentUserDto`              | `id:UUID`, `email:string 3..320`, `displayName:string 1..100`, `profileCompleted:boolean`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `ProfileDto`                  | `userId:UUID`, `legalName:string? 1..100`, `introduction:string? <=2000`, `desiredRoles:string[0..10]`, `desiredIndustries:string[0..10]`, `desiredLocations:string[0..10]`(각 항목 1..100), `expectedGraduationDate:LocalDate?`, `profileCompleted:boolean`, `missingCompletionItems:ProfileCompletionItem[]`, `version:long`, `createdAt:Instant`, `updatedAt:Instant`                                                                                                                                                                                                                   |
| `EducationDto`                | `id:UUID,userId:UUID,schoolName:string 1..200,major:string? <=200,degree:string? <=100,educationStatus:EducationStatus,admissionDate:LocalDate?,graduationDate:LocalDate?,gpa:decimal? 0..10,gpaScale:decimal? 0.01..10,isPrimary:boolean,description:string? <=5000,version:long,createdAt:Instant,updatedAt:Instant`                                                                                                                                                                                                                                                                     |
| `CertificationDto`            | `id:UUID,userId:UUID,name:string 1..200,issuer:string? <=200,credentialNumber:string? <=200,acquiredDate:LocalDate?,expiresAt:LocalDate?,description:string? <=5000,evidenceDocumentId:UUID?,version:long,createdAt:Instant,updatedAt:Instant`                                                                                                                                                                                                                                                                                                                                             |
| `LanguageScoreDto`            | `id:UUID,userId:UUID,testName:string 1..100,score:string 1..100,grade:string? <=100,testedAt:LocalDate?,expiresAt:LocalDate?,evidenceDocumentId:UUID?,version:long,createdAt:Instant,updatedAt:Instant`                                                                                                                                                                                                                                                                                                                                                                                    |
| `AwardDto`                    | `id:UUID,userId:UUID,name:string 1..200,organizer:string? <=200,awardedAt:LocalDate?,description:string? <=5000,evidenceDocumentId:UUID?,version:long,createdAt:Instant,updatedAt:Instant`                                                                                                                                                                                                                                                                                                                                                                                                 |
| `CareerDto`                   | `id:UUID,userId:UUID,organization:string 1..200,position:string? <=200,employmentType:string? <=50,startedAt:LocalDate?,endedAt:LocalDate?,isCurrent:boolean,responsibilities:string? <=20000,achievements:string? <=20000,version:long,createdAt:Instant,updatedAt:Instant`                                                                                                                                                                                                                                                                                                               |
...
| `DocumentDetailDto`           | `DocumentSummaryDto` + `originalFilename:string 1..255,checksumSha256:string(64 lowercase hex),pageCount:int? >=1,characterCount:int? >=0,parserName:string? <=100,parserVersion:string? <=50,parsedAt:Instant?`                                                                                                                                                                                                                                                                                                                                                                           |
| `DocumentTextDto`             | `documentId:UUID,text:string 0..500000,characterCount:int >=0,manualTextProvided:boolean,sourceRevision:long,version:long,updatedAt:Instant`                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `JobSummaryDto`               | `id:UUID,companyName:string? <=200,title:string? <=300,positionName:string? <=300,status:JobStatus,extractionStatus:JobExtractionStatus,submittedAt:Instant?,deadlineAt:Instant?,deadlineSource:DeadlineSource,latestFitScore:decimal? 0..100,analysisOutdated:boolean,outdatedReasons:OutdatedReason[],coverLetterStatus:CoverLetterStatus?,interviewPreparationCount:int >=0,version:long,createdAt:Instant,updatedAt:Instant`                                                                                                                                                           |
| `JobDetailDto`                | `JobSummaryDto` + `sourceUrl:string 1..2048,canonicalUrl:string 1..2048,roleCategory:string? <=100,employmentType:string? <=100,location:string? <=200,descriptionText:string? <=200000,descriptionSource:JobDescriptionSource?,extractionError:SafeErrorDto?,closedAt:Instant?,closedReason:ClosedReason?,latestAnalysis:JobAnalysisSummaryDto?,coverLetterId:UUID?,latestQuestionSetId:UUID?,latestMockSessionId:UUID?`                                                                                                                                                                  |
| `JobAnalysisSummaryDto`       | `id:UUID,analysisVersion:int >=1,eligibility:Eligibility,fitScore:decimal? 0..100,analysisOutdated:boolean,outdatedReasons:OutdatedReason[],createdAt:Instant,agentRunId:UUID`                                                                                                                                                                                                                                                                                                                                                                                                             |
| `JobAnalysisDetailDto`        | `JobAnalysisSummaryDto` + `jobContentHash:string,profileSnapshotHash:string,evidenceSnapshotHash:string,scoreRubricVersion:string,scoreBreakdown:ScoreCriterionDto[],requiredQualifications:RequirementItemDto[],preferredQualifications:RequirementItemDto[],responsibilities:RequirementItemDto[],strengths:string[0..20],gaps:string[0..20],matchedEvidenceRefs:EvidenceRefDto[],analysisSummary:string? <=10000`                                                                                                                                                                       |
...
| `CoverLetterQuestionDto`      | `id:UUID,questionOrder:int 1..20,questionText:string 1..2000,maxLength:int? 1..10000,memo:string? <=5000,currentAnswer:CoverLetterAnswerVersionDto?,latestVerification:VerificationDto?,version:long,deletedAt:Instant?`                                                                                                                                                                                                                                                                                                                                                                   |
| `CoverLetterAnswerVersionDto` | `id:UUID,questionId:UUID,parentVersionId:UUID?,restoredFromVersionId:UUID?,versionNo:int >=1,contentJson:TipTapDocumentDto,plainText:string 0..20000,characterCount:int 0..20000,sourceType:CoverLetterVersionSource,promptVersion:string? <=50,modelTier:ModelTier?,isCurrent:boolean,createdBy:AnswerCreatedBy,createdAt:Instant`                                                                                                                                                                                                                                                        |
| `VerificationDto`             | `id:UUID,answerVersionId:UUID,status:VerificationStatus,issues:VerificationIssueDto[],suggestions:string[0..20]`(항목 1..1000), `verifiedClaims:VerifiedClaimDto[],evidenceRefs:EvidenceRefDto[],agentRunId:UUID?,createdAt:Instant`                                                                                                                                                                                                                                                                                                                                                       |
| `CoverLetterDetailDto`        | `CoverLetterSummaryDto` + `questions:CoverLetterQuestionDto[]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `ResearchSourceDto`           | `id:UUID,topic:ResearchTopic,sourceUrl:string 1..2048,title:string? <=500,sourceType:ResearchSourceType,publishedAt:Instant?,retrievedAt:Instant,snippet:string? <=2000,reliabilityNotice:string 1..500,providerRank:int? >=1,contentHash:string? 64 lowercase hex`                                                                                                                                                                                                                                                                                                                        |
...
| `InterviewQuestionDto`        | `id:UUID,questionOrder:int 1..20,questionType:InterviewQuestionType,questionText:string 1..2000,intent:string? <=2000,evaluationPoints:string[0..20]`(항목 1..500), `answerGuide:string? <=10000,followUpQuestions:string[0..10]`(항목 1..2000), `relatedEvidenceRefs:EvidenceRefDto[],sourceRefs:ResearchSourceRefDto[],sourceBased:boolean,currentAnswer:InterviewAnswerVersionDto?,latestFeedback:InterviewFeedbackDto?`                                                                                                                                                                |
| `QuestionSetDetailDto`        | `QuestionSetSummaryDto` + `research:ResearchRunDto,questions:InterviewQuestionDto[]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `InterviewAnswerVersionDto`   | `id:UUID,questionId:UUID,parentVersionId:UUID?,versionNo:int >=1,content:string 1..20000,sourceType:CoverLetterVersionSource,isCurrent:boolean,createdAt:Instant`                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `InterviewFeedbackDto`        | `id:UUID,answerVersionId:UUID,status:InterviewFeedbackStatus,scores:FeedbackScoreDto[],strengths:string[0..20]`(항목 1..1000), `weaknesses:string[0..20]`(항목 1..1000), `suggestions:string[0..20]`(항목 1..1000), `revisedExample:string? <=10000,agentRunId:UUID,createdAt:Instant`                                                                                                                                                                                                                                                                                                     |
...
| `MockFeedbackDto`             | `status:MockFeedbackStatus,agentRunId:UUID?,items:MockFeedbackItemDto[],sessionSummary:string? <=10000,safeError:SafeErrorDto?`                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `AgentStepDto`                | `id:UUID,stepKey:string 1..100,scopeKey:string? <=100,stepOrder:int >=1,status:AgentStepStatus,attempt:int 1..maxAttempts,maxAttempts:int 1..3,reusedStepId:UUID?,modelTier:ModelTier?,startedAt:Instant?,completedAt:Instant?,safeError:SafeErrorDto?`                                                                                                                                                                                                                                                                                                                                    |
...
| `AgentRunDetailDto`           | `AgentRunSummaryDto` + `retryOfRunId:UUID?,rootRunId:UUID,runAttemptNo:int >=1,durationMs:long? >=0,startedAt:Instant?,completedAt:Instant?,safeError:SafeErrorDto?,partialResult:PartialResultDto?,steps:AgentStepDto[]`                                                                                                                                                                                                                                                                                                                                                                  |
| `DashboardDto`                | `generatedAt:Instant`, `profile:{completed:boolean,completionPercent:int 0..100,missingItems:ProfileCompletionItem[]}`, `documents:{processingCount:int >=0,needsActionCount:int >=0}`, `jobs:{inProgressCount:int >=0,submittedCount:int >=0,closingSoon:JobSummaryDto[0..5]}`, `coverLetters:{warningCount:int >=0,recent:CoverLetterSummaryDto[0..5]}`, `mockInterviews:{recent:MockSessionSummaryDto[0..5]}`, `agentRuns:{activeCount:int >=0,recent:AgentRunSummaryDto[0..5]}`                                                                                                        |
...
| `DELETE /account`             | Session/필수     | JSON `currentPassword:string 1..72 bytes`                                                                                                              | I         | 202 `AccountDeletionAcceptedDto`; 즉시 `WITHDRAWN`, 모든 session 폐기 | 400/401/403/409                            | 비동기 deletion task/Agent Run 없음 | `/settings/account`, 접수 receipt  |
| `GET /dashboard`              | Session/불필요   | 없음                                                                                                                                                   | 없음      | 200 `DashboardDto`                                                    | 401                                        | 동기 read projection/없음           | `/dashboard`                       |

signup/login 성공 뒤 응답의 새 CSRF 값을 client가 교체한다. 별도 재조회도 허용하지만 이전 anonymous token을 재사용하지 않는다. `profileCompleted`는 client 입력이 아니며 `legalName`, 희망 직무·산업·지역 각 1개 이상, 대표 학력 1개를 모두 만족할 때 true다. introduction과 문서 업로드는 완료 조건이 아니다.
...
- MVP active embedding은 현재 기본 모델과 맞춘 1536차원으로 고정하고 `vector(1536)`을 사용한다. boot 시 provider 반환 dimension·configured model metadata가 다르면 fail fast한다.
- 모든 query는 `user_id`, active document, `embedding_model`, `embedding_dimension=1536`, active generation을 조건으로 한다.
...
|    5 | documents·evidence           | document/text/chunk `vector(1536)`, two status axes, owner composite FK, Object outbox                       |
...
| `MOCK_INTERVIEW_FEEDBACK`   | `LOAD_SESSION_SNAPSHOT → ANALYZE_TURNS → SYNTHESIZE_SESSION_FEEDBACK → VALIDATE_FEEDBACK → PERSIST_FEEDBACK`                                                                                                                                                                 | session/message IDs, transcript hash                                                                | session feedback ID/status                                                         |
...
HIGH_QUALITY 허용 workflow는 cover letter 생성·검증, interview answer feedback, mock session feedback이다. job 분석, document/job extraction, interview preparation은 ECONOMY/BALANCED만 허용한다. endpoint·OpenAPI·ModelRouter가 같은 allowlist를 사용하며 provider/model 실명은 일반 API에 노출하지 않고 `highestModelTierUsed`만 표시한다.
...
| `/signup`, `/login` authenticated    | 위 profile 기준 destination으로 replace        |
...
아래 6개만 제품 소유자 결정이 필요하다. 선택 전에도 구현자는 권장안의 구조와 대안 영향을 평가할 수 있지만 관련 공개 계약·migration은 만들지 않는다.
...
1. 제품 소유자가 8장의 6개 항목과 나머지 권장안을 승인·수정한다.
