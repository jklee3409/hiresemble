...
Flyway V1 보존과 V2 P1 schema의 빈 DB·upgrade 경로를 실제 PostgreSQL에서 검증한다.
...
- migration 적용 순서와 schema 범위가 P1 계약을 넘지 않는지 독립 DB에서 확인한다.
...
- 적용 이력 V1을 test 편의를 위해 수정하거나 H2로 대체하지 않는다.
