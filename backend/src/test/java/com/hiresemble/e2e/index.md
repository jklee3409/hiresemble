...
격리 PostgreSQL·Object Storage·Spring·Vue·Chromium을 한 수명주기로 실행하는 P4~P7 cross-stack 검증 harness를 관리한다.
...
- Browser 시나리오는 Frontend의 `documents.actual.spec.ts`, `jobs.actual.spec.ts`, `job-analysis.actual.spec.ts`와 `cover-letter.actual.spec.ts`에 있다.
- Gradle task는 [`../../../../../../build.gradle.kts`](../../../../../../build.gradle.kts)의 `p4BrowserE2eTest`부터 `p7BrowserE2eTest`다.
