...
capability별 단일 Bean, hidden retry 0, strict output, vector shape, 가격 row와 network-disabled 기본 회귀를 고정한다. 실제 Spring OpenAI SDK 요청 객체를 capture해 runtime registry의 schema·strict flag·store/tool 설정과 schema rejection 분류를 확인하고 통제 가능한 지연 model로 wall-clock timeout과 worker 종료를 검증한다.
