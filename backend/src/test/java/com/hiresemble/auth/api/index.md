...
P1 다섯 인증 endpoint와 계정 닉네임 변경 회귀, P1~P7 OpenAPI의 실제 Spring 통합 계약을 검증한다.

## 주요 파일 및 하위 디렉터리

- [`AuthIntegrationTest.java`](AuthIntegrationTest.java): 가입·로그인·CSRF·Session·오류·Request ID 통합
- [`OpenApiContractTest.java`](OpenApiContractTest.java): 정확히 100 operations/74 paths, DTO·enum·nullability, 금지 경로 부재와 Swagger UI 접근
