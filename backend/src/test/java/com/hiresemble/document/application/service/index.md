# Document 테스트 Service package 안내

## 디렉터리 목적

`com.hiresemble.document.application.service` package는 Document text pipeline use case와 transaction의 회귀·계약 테스트를 소유한다.

## 주요 파일 및 하위 디렉터리

| 파일 | 역할 |
| ---- | ---- |
| [DocumentTextPipelineTest.java](DocumentTextPipelineTest.java) | Service 책임 구현 |
| [progress.md](progress.md) | 이 package의 이동·검증 이력 |

## 구성 요소 역할

- Document text pipeline use case와 transaction 경계를 검증한다.
