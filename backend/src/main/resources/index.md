...
| 경로                                             | 역할                                                              |
| ------------------------------------------------ | ----------------------------------------------------------------- |
| [`application.yml`](application.yml)             | datasource, Session·AI·Document·Job·Cover Letter·Career Artifact와 OpenAPI 설정 |
| [`application-local.yml`](application-local.yml) | 로컬 개발 전용 idempotency HMAC와 Career Artifact 활성 설정               |
| [`db/`](db/)                                     | DB 관리 리소스의 상위 경계                                        |
| [`db/migration/`](db/migration/)                 | Flyway가 순서대로 적용하는 SQL migration                          |

## 구성 요소 역할

- `application.yml`은 선택적 `.env`를 읽고 Session·Cookie, versioned HMAC, bounded Agent worker, AI workflow와 기본 비활성 Career Artifact typed 설정을 정의한다.
- `application-local.yml`은 명시적 local profile에서만 개발 전용 HMAC 키를 제공하며 비로컬 환경의 secret 계약을 대체하지 않는다.
