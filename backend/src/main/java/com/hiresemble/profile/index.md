...
P2 기본 프로필, 학력·자격증·어학·수상·경력·사용자 직접 대외활동과 P4 Document에서 추출되는 근거를 사용자 소유 resource로 관리하고, 공고 분석·자기소개서·면접에 owner-scoped 현재 `VERIFIED` snapshot을 제공한다.
...
- 공고 분석에는 canonical profile hash와 `VERIFIED` evidence의 ID·version·hash·provenance만 제공하며 profile entity와 원문 전체를 노출하지 않는다.
