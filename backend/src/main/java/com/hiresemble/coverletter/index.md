...
하나의 공고에 active 자기소개서를 최대 하나로 제한하고, 답변·검증·근거는 immutable history로 보존한다. AI workflow는 application query/command port만 사용하며 JDBC row나 공개 DTO에 의존하지 않는다. generation·verification v4 snapshot은 사용자가 선택한 exact OpenAI model을 고정하고 current VERIFIED evidence·owner-scoped sibling current answer·문항 memo를 bounded AI Context로 제공하며 공개 lifecycle은 그대로 유지한다.
