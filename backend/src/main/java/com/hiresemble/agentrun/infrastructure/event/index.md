# Agent Run Event package 안내

## 디렉터리 목적

com.hiresemble.agentrun.infrastructure.event package는 transaction 이후 event 전달과 실행 연결을 소유한다.

## 주요 파일 및 하위 디렉터리

| 파일 | 역할 |
| ---- | ---- |
| [AgentRunEventBus.java](AgentRunEventBus.java) | Event 책임 구현 |
| [progress.md](progress.md) | 이 package의 이동·검증 이력 |

## 구성 요소 역할

- transaction 이후 event 전달과 실행 연결을 소유한다.
