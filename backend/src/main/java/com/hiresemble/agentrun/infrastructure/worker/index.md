# Agent Run Worker package 안내

## 디렉터리 목적

com.hiresemble.agentrun.infrastructure.worker package는 DB 상태를 기준으로 비동기 작업과 복구 실행을 조정한다.

## 주요 파일 및 하위 디렉터리

| 파일 | 역할 |
| ---- | ---- |
| [AgentRunDispatcher.java](AgentRunDispatcher.java) | Worker 책임 구현 |
| [AgentRunReconciler.java](AgentRunReconciler.java) | Worker 책임 구현 |
| [progress.md](progress.md) | 이 package의 이동·검증 이력 |

## 구성 요소 역할

- DB 상태를 기준으로 비동기 작업과 복구 실행을 조정한다.
