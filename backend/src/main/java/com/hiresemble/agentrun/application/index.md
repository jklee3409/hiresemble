...
- `WorkflowLauncher*`: budget reserve와 Run 접수
- `AgentRun*Port`, `AgentStepCheckpointPort`: 상태·checkpoint 경계
- `AgentRunLeaseHeartbeatPort`: transaction 밖의 blocking gateway 호출 중 lease 유지 경계
- `BudgetReservationPort`, `UsageRecorderPort`: 비용 원자성 경계
- `DomainResultApplyPort`, `ResourceCompensationPort`: 후속 domain 적용 경계
- [`progress.md`](progress.md): application 구현 상태

## 구성 요소 역할

transaction과 owner·stateVersion·lineage 규칙을 조정하며 AI 구현 세부사항과 domain repository를 분리한다.
