# Agent Run SSE package 안내

## 디렉터리 목적

com.hiresemble.agentrun.api.sse package는 Agent Run 상태를 snapshot-first SSE stream으로 전달한다.

## 주요 파일 및 하위 디렉터리

| 파일 | 역할 |
| ---- | ---- |
| [AgentRunSseService.java](AgentRunSseService.java) | SSE 책임 구현 |
| [progress.md](progress.md) | 이 package의 이동·검증 이력 |

## 구성 요소 역할

- Agent Run 상태를 snapshot-first SSE stream으로 전달한다.
