...
Controller는 인증 사용자와 HTTP parameter를 application service로 전달하고 Entity·내부 hash·provider 정보를 노출하지 않는다. 목록 DTO는 owner의 active 공고에 실제 존재하는 연도·상하반기 선택지를 함께 반환하고 preset 기간과 직접 시작일 필터를 분리한다. `JobDetailDto.automaticAnalysis`는 고정 품질, 안전한 상태·Run ID·오류만 additive projection으로 반환한다.
