...
ControllerAdvice와 Security가 함께 사용하는 공개 오류 DTO·생성 규칙을 소유한다. 공통 OpenAPI metadata는 형제 `openapi` package가 담당한다.

## [2026-07-23] Session Summary (책임별 backend package 세분화)

- What was done:
  - backend/src/main/java/com/hiresemble/common/api 영역의 기존 Java 책임을 실제 하위 package와 추적 문서에 반영했다.
