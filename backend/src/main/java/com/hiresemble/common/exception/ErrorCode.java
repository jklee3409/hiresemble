...
public enum ErrorCode {
    ...
    GITHUB_CONNECTION_STATE_INVALID_OR_EXPIRED(HttpStatus.BAD_REQUEST, "GitHub 연결 요청이 만료되었거나 유효하지 않습니다."),
    ...
    GITHUB_CONNECTION_REQUIRED(HttpStatus.CONFLICT, "활성 GitHub App 연결이 필요합니다."),
    GITHUB_INSTALLATION_SUSPENDED(HttpStatus.CONFLICT, "GitHub App 설치가 일시 중지되었습니다."),
    GITHUB_INSTALLATION_REVOKED(HttpStatus.CONFLICT, "GitHub App 설치가 해제되었습니다."),
    PASSWORD_REUSE_NOT_ALLOWED(HttpStatus.CONFLICT, "현재 비밀번호와 다른 새 비밀번호를 입력해 주세요."),
    CAREER_ARTIFACT_GENERATION_IN_PROGRESS(HttpStatus.CONFLICT, "이미 생성 작업이 진행 중입니다."),
    CAREER_ARTIFACT_ARCHIVED(HttpStatus.CONFLICT, "보관된 Career Artifact는 변경할 수 없습니다."),
    CAREER_ARTIFACT_VERSION_NOT_READY(HttpStatus.CONFLICT, "다운로드할 수 있는 파일 버전이 아닙니다."),
    INSUFFICIENT_VERIFIED_EXPERIENCE(HttpStatus.CONFLICT, "승인된 경력 근거가 한 개 이상 필요합니다."),
    GITHUB_SOURCE_NOT_ACCESSIBLE(HttpStatus.UNPROCESSABLE_ENTITY, "공개 GitHub source에 접근할 수 없습니다."),
    GITHUB_INSTALLATION_NOT_ACCESSIBLE(HttpStatus.UNPROCESSABLE_ENTITY, "현재 GitHub 사용자가 이 설치에 접근할 수 없습니다."),
    GITHUB_APP_PERMISSION_MISMATCH(HttpStatus.UNPROCESSABLE_ENTITY, "GitHub App 권한을 Metadata read와 Contents read로 설정해 주세요."),
    ...
    GITHUB_APP_NOT_CONFIGURED(HttpStatus.SERVICE_UNAVAILABLE, "Private GitHub 연결이 구성되지 않았습니다."),
    GITHUB_UPSTREAM_AUTHENTICATION_FAILED(HttpStatus.SERVICE_UNAVAILABLE, "GitHub 인증을 확인하지 못했습니다."),
