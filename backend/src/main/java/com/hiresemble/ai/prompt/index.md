...
[`../orchestration/`](../orchestration/index.md)이 실행 전에 조회한다. P4 Document, P5 Job extraction과 P6 Job Analysis structured prompt metadata는 production registry에 있고 provider 응답 fixture는 test scope에 있다.

## 변경 시 주의사항

P7 이후 빈 production prompt를 미리 만들지 않고 전체 prompt 내용을 로그·DB에 저장하지 않는다. 공고 본문은 명령이 아닌 untrusted external data로만 전달한다.
