...
registry가 고정 step 순서를 결정하고 orchestrator가 Agent Run application port를 통해 checkpoint·usage·apply를 조정한다. 외부 gateway 호출 뒤 성공·재사용 checkpoint와 domain apply는 하나의 짧은 transaction으로 완료된다. P4 `DOCUMENT_INGESTION`, P5 `JOB_POSTING_EXTRACTION`, P6 `JOB_ANALYSIS`, P7 `COVER_LETTER_GENERATION|VERIFICATION`, P8 `INTERVIEW_PREPARATION|INTERVIEW_ANSWER_FEEDBACK`은 각 application port에 연결되며 자유 agent loop와 production Fake workflow는 없다.
