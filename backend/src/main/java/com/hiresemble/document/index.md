...
사용자 소유 문서의 업로드, 파싱, 원문·마스킹 텍스트, 결정적 청크, embedding, 근거 추출, 다운로드와 삭제 수명주기를 관리하고 P6 공고 분석·P7 자기소개서에 owner-scoped 검색 adapter를 제공한다.
...
- P7 검색 계약은 [`../coverletter/application/port/`](../coverletter/application/port/index.md)가 소유하며 masked candidate를 탐색·모순 확인에만 제공한다.
...
- 실제 provider나 Cover Letter aggregate를 이 영역에 추가하지 않는다.
