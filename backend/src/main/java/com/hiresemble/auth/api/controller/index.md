# 인증 Controller package 안내

## 디렉터리 목적

com.hiresemble.auth.api.controller package는 인증 context와 HTTP 입력·출력을 application use case에 연결한다.

## 주요 파일 및 하위 디렉터리

| 파일                                             | 역할                              |
| ------------------------------------------------ | --------------------------------- |
| [AccountController.java](AccountController.java) | 표시 이름·비밀번호 변경과 terminal 탈퇴 HTTP 계약 |
| [AuthController.java](AuthController.java)       | Controller 책임 구현              |
| [progress.md](progress.md)                       | 이 package의 이동·검증 이력       |

## 구성 요소 역할

- 인증 context와 HTTP 입력·출력을 application use case에 연결한다.
