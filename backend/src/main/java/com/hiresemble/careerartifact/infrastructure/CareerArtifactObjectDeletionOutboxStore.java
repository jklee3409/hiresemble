...
@Repository
public class CareerArtifactObjectDeletionOutboxStore {

    private final JdbcClient jdbc;

    public CareerArtifactObjectDeletionOutboxStore(JdbcClient jdbc) {
        this.jdbc = jdbc;
    }

    @Transactional
    public UUID enqueueArtifact(
            UUID userId,
            UUID artifactId,
            UUID versionId,
            String storageKey,
            Instant now) {
        return enqueue(
                userId, artifactId, versionId, storageKey, "ARTIFACT_DELETE", now);
    }

    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public UUID enqueueOrphan(
            UUID userId,
            UUID artifactId,
            UUID versionId,
            String storageKey,
            Instant now) {
        return enqueue(
                userId, artifactId, versionId, storageKey,
                "ORPHAN_UPLOAD_COMPENSATION", now);
    }
    ...
    private UUID enqueue(
        ...
        return jdbc.sql("""
                        INSERT INTO career_artifact_object_deletion_outbox (
                          id,user_id,career_artifact_id,artifact_version_id,storage_key,
                          reason,status,attempt_count,next_attempt_at,claim_token,
                          lease_expires_at,last_error_code,created_at,completed_at
                        ) VALUES (
                          :id,:userId,:artifactId,:versionId,:storageKey,:reason,'PENDING',
                          0,:now,NULL,NULL,NULL,:now,NULL)
                        ON CONFLICT (storage_key,reason)
                          WHERE status IN ('PENDING','PROCESSING')
                        DO UPDATE SET next_attempt_at=LEAST(
                          career_artifact_object_deletion_outbox.next_attempt_at,
    ...
    public Optional<ClaimedDeletion> claimDue(Instant now, Duration lease) {
        jdbc.sql("""
                        UPDATE career_artifact_object_deletion_outbox
        ...
        return jdbc.sql("""
                        WITH candidate AS (
                          SELECT id FROM career_artifact_object_deletion_outbox
                          WHERE status='PENDING' AND next_attempt_at<=:now
                          ORDER BY next_attempt_at,id
                          FOR UPDATE SKIP LOCKED LIMIT 1)
                        UPDATE career_artifact_object_deletion_outbox target
    ...
    public boolean markFailed(
        ...
        return jdbc.sql("""
                        ...
                        SET status=:status,claim_token=NULL,lease_expires_at=NULL,
                            last_error_code='CAREER_ARTIFACT_OBJECT_DELETE_FAILED',
    ...
    private ClaimedDeletion claim(ResultSet rs, int row) throws SQLException {
        return new ClaimedDeletion(
                ...
                rs.getObject("artifact_version_id", UUID.class),
