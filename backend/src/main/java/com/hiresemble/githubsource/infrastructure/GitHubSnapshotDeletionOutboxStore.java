...
@Repository
public class GitHubSnapshotDeletionOutboxStore {

    private final JdbcClient jdbc;

    public GitHubSnapshotDeletionOutboxStore(JdbcClient jdbc) {
        this.jdbc = jdbc;
    }

    public UUID enqueueSource(
            UUID userId, UUID sourceId, UUID snapshotId, String storageKey, Instant now) {
    ...
    private UUID enqueue(
        ...
        return jdbc.sql("""
                        INSERT INTO github_snapshot_object_deletion_outbox (
                            ...
                            :id,:userId,:sourceId,:snapshotId,:storageKey,:reason,'PENDING',
                            0,:now,NULL,NULL,NULL,:now,NULL
                        )
                        ON CONFLICT (storage_key,reason)
                            WHERE status IN ('PENDING','PROCESSING')
                        DO UPDATE SET next_attempt_at=LEAST(
                            github_snapshot_object_deletion_outbox.next_attempt_at,
    ...
    public Optional<ClaimedDeletion> claimDue(Instant now, Duration lease) {
        ...
        return jdbc.sql("""
                        WITH candidate AS (
                            SELECT id FROM github_snapshot_object_deletion_outbox
                            WHERE status='PENDING' AND next_attempt_at<=:now
                            ORDER BY next_attempt_at,id
                            FOR UPDATE SKIP LOCKED LIMIT 1
                        )
                        UPDATE github_snapshot_object_deletion_outbox target
    ...
    public boolean markFailed(
        ...
        return jdbc.sql("""
                        ...
                        SET status=:status,claim_token=NULL,lease_expires_at=NULL,
                            last_error_code='GITHUB_SNAPSHOT_DELETE_FAILED',
