...
이 디렉터리는 Hiresemble의 Vue 기반 단일 페이지 애플리케이션과 프론트엔드 개발·검증 설정을 관리한다. 현재 P1 인증·Session 기반과 P2 프로필·온보딩·direct evidence 사용자 흐름이 구현되어 있다.
...
| [`e2e/`](e2e/)                                 | Playwright 실제 Cookie·CSRF·두 사용자 프로필 여정           |
...
- `src/shared/api`와 `src/stores`가 Session Cookie·CSRF, 인증 상태와 typed profile transport를 관리한다.
- `src/layouts`, `src/features/profile`, `src/pages`는 인증 shell과 P2 프로필·온보딩·evidence 흐름을 제공한다.
...
- Dashboard 집계, 문서 연결·업로드와 AI route는 아직 구현되지 않았으므로 안내·shell을 제품 기능 완료로 기록하지 않는다.
