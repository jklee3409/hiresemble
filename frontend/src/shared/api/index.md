...
- [`documentContracts.ts`](documentContracts.ts): 문서·parse·evidence enum과 공개 DTO Zod 계약
- [`documentApi.ts`](documentApi.ts): 문서 공개 API 8개 multipart·version·idempotency consumer
- [`jobContracts.ts`](jobContracts.ts): 업무·추출 상태와 P5 공개 DTO의 strict Zod 계약
- [`jobApi.ts`](jobApi.ts): Job 공개 API 7개, filter·version·idempotency consumer
...
- Agent Run 생성은 별도 공개 client로 만들지 않고 document/job 202 응답의 Run ID를 사용한다. storage key·hash·provider metadata와 P6 분석 DTO를 type에 추가하지 않는다.
