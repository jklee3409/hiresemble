...
이 디렉터리는 공개 Landing부터 P1 인증·P8 면접 준비까지 route table, browser history, 공통 문서 제목, guard와 안전한 `returnTo` 검증을 관리한다.

## 주요 파일 및 하위 디렉터리

| 경로                                   | 역할                                                                                        |
| -------------------------------------- | ------------------------------------------------------------------------------------------- |
| [`index.ts`](index.ts)                 | 인증·profile·experience·Agent Run·Document·Job·Cover Letter·Interview route, scroll과 guard |
...
- `index.ts`는 anonymous `/` 공개 Landing, 인증, onboarding, 재방문 가능한 `/guide`, dashboard, `/profile/activities`와 `/profile/experiences`를 포함한 8개 profile section, lazy 업무 route와 Job analysis/cover-letter/interview child, 질문 set 목록·상세 route를 등록한다.
...
- `returnTo`는 dashboard·onboarding·등록된 profile·canonical experience·Agent Run·Document·Job·Cover Letter의 same-origin path만 허용한다.
