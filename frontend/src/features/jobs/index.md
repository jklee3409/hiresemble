...
P5 Job URL filter, Vue Query, form validation, presentation, Agent Run monitor와 409 version conflict 재적용을 소유한다.

## 주요 파일 및 하위 디렉터리

- `filters.ts`: URL query parse·canonicalization과 sort allowlist
- `queries.ts`: user-scoped list/detail/mutation query와 cleanup
...
URL query를 목록 filter의 원천으로 사용하고 terminal·WAITING_USER Run event에서 Job과 Agent Run query를 invalidate한다.
...
업무 상태와 추출 상태를 합치지 않고 P6 analysis action·DTO를 선행 추가하지 않는다.
