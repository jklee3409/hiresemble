...
Agent Run 목록 filter, Vue Query, 안전한 상태 표현, Progress Drawer와 Document·Job resource SSE 복구 상태 machine을 소유한다.
...
Backend DTO와 stateVersion을 그대로 소비하며 연결 상태와 Run business 상태를 분리한다. Document·Job stream은 terminal·WAITING_USER에서 관련 REST query만 invalidate한다.
