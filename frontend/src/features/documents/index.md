...
REST Document 상태를 최종 원천으로 사용하고 SSE terminal·WAITING_USER에는 document/text/evidence query를 invalidate한다. 문서 소재 panel은 PENDING·VERIFIED·REJECTED를 사용자용 검토 상태로 변환하고 원본 삭제와 활용 제외를 구분하며, 목록은 5개씩 나눠 보여 주되 요약 수치와 일괄 승인은 전체 소재를 대상으로 한다. 원본 미리보기는 PDF만 페이지 단위로 그리고, 다른 형식과 렌더링 실패는 추출한 텍스트로 되돌린다.
