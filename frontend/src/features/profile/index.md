...
- [`ProfileTabs.vue`](ProfileTabs.vue): 항목별 icon과 현재 위치를 강조하는 desktop 세로 navigation·mobile selector
...
- GitHub 연동은 `이력서·자료` 영역의 `/integrations`가 소유하므로 이 navigation에는 두지 않는다.
