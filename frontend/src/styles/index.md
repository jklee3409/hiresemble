...
- `main.css`는 Tailwind utility 생성과 함께 canvas·surface·text·border·brand·semantic color, radius·shadow·content width·typography token을 선언한다.
- 최소 reset, focus ring, reduced motion, button·control·alert·status·skeleton·pagination처럼 여러 화면이 공유하는 pattern만 전역으로 제공한다.
