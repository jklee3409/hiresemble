...
이 디렉터리는 Hiresemble의 핵심 사용자 여정과 responsive shell을 실제 브라우저에서 검증하는 Playwright 테스트를 관리한다. 현재 UI shell, P2 profile, P3 Agent Run fixture, P4 Document와 P5 Job 실제 Backend pipeline이 구현되어 있다.
...
| [`ui-shell.spec.ts`](ui-shell.spec.ts)                 | 1440·1024·768·390px overflow, desktop/sidebar와 mobile drawer fixture  |
...
현재 하위 디렉터리는 없고 responsive UI shell, P2 profile, P3 Agent Run, P4 Document와 P5 Job spec을 관리한다.
...
- UI shell fixture는 Backend 없이 인증 응답과 active Run 목록을 가로채 필수 viewport와 focus 복원을 검증한다.
